...
def test_admin_sees_system_logs(client):
    with db.connect() as conn:
        _insert(conn, level="ERROR", message="캡처 실패", traceback="Traceback ...")
    _login(client, "boss@test.co", "bosspass1234")
    res = client.get("/system/logs")
    assert res.status_code == 200
    assert "캡처 실패" in res.text
    assert "Traceback ..." in res.text


def test_system_logs_level_filter(client):
    with db.connect() as conn:
        _insert(conn, level="INFO", message="정보 기록")
        _insert(conn, level="ERROR", message="오류 기록")
    _login(client, "boss@test.co", "bosspass1234")
    page = client.get("/system/logs", params={"level": "ERROR"}).text
    assert "오류 기록" in page
    assert "정보 기록" not in page


def test_non_admin_cannot_see_system_logs(client):
    for email in ("viewer@test.co", "archiver@test.co"):
        _login(client, email)
        assert client.get("/system/logs").status_code == 403
...
def test_menu_visibility_by_role(client):
    # 관리자: 아카이빙 로그 + 시스템 로그 메뉴
    _login(client, "boss@test.co", "bosspass1234")
    page = client.get("/archives").text
    assert 'href="/logs"' in page
    assert 'href="/system/logs"' in page
    assert "아카이빙 로그" in page
    # viewer: 아카이빙 로그만, 시스템 로그 메뉴 없음
    viewer = TestClient(web_app.app)
    _login(viewer, "viewer@test.co")
    page = viewer.get("/archives").text
    assert 'href="/logs"' in page
    assert 'href="/system/logs"' not in page


def test_pending_cannot_reach_logs(client):
    """pending 은 미들웨어가 /pending 으로 보낸다 — 로그 화면 접근 불가."""
    _login(client, "pending@test.co")
    res = client.get("/logs", follow_redirects=False)
    assert res.status_code == 302
    assert res.headers["location"] == "/pending"
    res = client.get("/system/logs", follow_redirects=False)
    assert res.status_code == 302
    assert res.headers["location"] == "/pending"
