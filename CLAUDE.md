...
- **병렬 실행 — 읽기 전용 계열(기본)**: 조사·분석·검증 에이전트(deep-reasoner·
  docs-researcher·db-analyst·code-reviewer·test-runner·security-auditor)는 소스를
  수정하지 않고 출력 계약이 서로 다른 문서 경로이므로, 위임할 질문이 서로 독립이면
  한 턴에 병렬로 실행하는 것을 기본으로 한다(예: 업그레이드 전 deep-reasoner 내부
  영향도 조사와 docs-researcher 외부 마이그레이션 가이드 조사를 동시에).
- **병렬 실행 — worker(조건부)**: default-worker·task-worker·ui-worker의 병렬 위임은
  각 위임의 변경 허용 파일 목록이 서로소(disjoint)일 때만 허용한다(같은 워킹 트리를
  공유하므로 겹치면 상호 덮어쓰기). 병렬 worker의 완료 조건은 자기 변경 범위의 단위
  검증(포맷·린트·해당 모듈 테스트)으로 한정하고, 전체 테스트는 전원 완료 후
  test-runner 1회로 통합 실행하며, 리뷰는 통합 diff에 대해 code-reviewer 1회로
  수행한다(worker별 개별 리뷰 아님 — 리뷰 루프는 통합 diff 기준으로 동일 적용).
- **병렬 상한(§13.8)**: 동시 실행 서브 에이전트는 최대 4개. 병렬화는 벽시계 시간만
  줄이고 컨텍스트 소비는 늘린다 — N개 병렬 결과가 한꺼번에 부모 컨텍스트로 돌아오므로,
  반환 요약 상한(출력 계약)을 지키는 위임만 병렬로 묶는다(worker·읽기 전용 공통).
  서브 에이전트 병렬은 세션 내부의 세밀한 병렬(조사 팬아웃·서로소 파일 구현·PR 게이트
  검증)에 쓰고, 기능·리포 단위 굵은 병렬은 worktree 병렬 세션에 둔다(두 계층 혼용 금지).
