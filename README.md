...
- 컨테이너 대시보드는 내부적으로 0.0.0.0 바인딩이라 **인증이 항상 켜진다**
  (`WCCG_AUTH=off` 는 loopback 바인딩 전용). 첫 접속 시 `/signup` 에서 가입.
...
신규 가입(`/signup`)과 SSO 자동 생성 계정은 **보기 전용(viewer)** 으로
시작하며, 관리자가 헤더의 **사용자** 메뉴(`/system/users`)에서 권한을
조정한다. 차단하면 해당 사용자의 모든 세션이 즉시 로그아웃된다.
