...
| 역할 | 설명(=권한 프리셋) |
|---|---|
| `admin` (관리자) | 전체 권한 (아래 7개 권한 모두) |
| `archiver` (아카이브) | `view` + `archive` + `delete` |
...
세분 권한(`db.PERMISSIONS`)은 다음 7가지다.
...
삭제는 막기"(archiver − delete), "사용자 관리만 위임"(viewer + manage_users).
...
없다 (관리 잠김 방지). 개인 API Key(확장 토큰)의 보기/아카이브 권한도
사용자의 실효 권한에서 파생되므로, 오버라이드가 토큰 권한에도 즉시 반영된다.
