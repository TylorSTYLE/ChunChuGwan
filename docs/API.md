...
REST API·확장 캡처의 정본 설계 규칙은 [`.claude/rules/api-extension.md`](../.claude/rules/api-extension.md)
에 있다. 아래는 `web/api_routes.py`·`ingest.py`·`extension/**` 을 만질 때 지켜야 하는 요약이다.

- **쓰기는 코어 모듈을 통해서만 (아키텍처 원칙 1).** REST 쓰기 엔드포인트
  (`/archive`·`/crawl`·`/auth-profiles`·`/ingest`)도 `storage.py`+`db.py` 코어를
  재사용한다 — API 계층이 파일·DB 를 직접 조작하지 않는다.
- **`/api/v1` 은 개인 API Key 전용.** 사용자 귀속 토큰만 인증 대상이고 관리자 시스템
  키는 제외한다. 토큰 권한은 매 요청 소유자의 실효 권한으로 재평가하므로 역할 강등·
  권한 회수가 즉시 반영된다(`use_api_keys` 상실 시 그 사용자의 키가 모두 401). 발급·
  저장·인증보호 상세는 아래 [인증과 엔드포인트](#인증과-엔드포인트) 절.
- **적재(ingest)는 신뢰 경계.** 확장이 브라우저에서 직접 캡처한 산출물을 받되 서버는
  그 URL 을 다시 가져오지 않고(`client_captured`), 스냅샷에 "브라우저 캡처·불완전"
  뱃지로 로그인 상태 캡처의 민감 정보 노출 가능성을 고지한다.
- **사설 IP·루프백 게이트 (아키텍처 원칙 7).** REST 아카이빙도 코어 게이트를 거친다 —
  사설 호스트를 네트워크 태그 없이 요청하면 422(`needs_network_tag`).
- **확장은 진입점만 안다.** 확장이 여는 대시보드 딥링크는 `/extension/*` 진입점만 쓰고
  서버가 정식 SPA 화면으로 302 한다 — 화면 경로가 바뀌어도 확장은 무관하다. 이전
  (migration) 채널(`/api/migration/*`)은 `/api/v1` 과 별개이며 이전 토큰으로 인증한다.
- **확장 버전은 서버와 독립, semver 자동 상향.** 확장(`extension/**`)을 바꿀 때마다
  영향도로 `manifest.json` 의 `version` 을 올린다 — major=재설치 필요(권한·계약·저장
  형식 파괴), minor=하위호환 신기능, patch=버그·문구·리팩토링.
