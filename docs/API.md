...
위 시스템 키와 별개로, 사용자가 **개인 API Key**(`/settings/api-keys`,
크롬 확장용 사용자 귀속 토큰)를 직접 발급할 수 있다. 발급·사용 모두 세분 권한
`use_api_keys` 가 필요하고(빌트인 기본은 관리자·아카이브 관리·아카이브 보유,
보기 전용 제외), 토큰 권한은 소유자의 현재 실효 권한으로 매 요청 재평가되므로
역할 강등·권한 회수가 즉시 반영된다 (권한을 잃으면 401). 권한 체계는
`docs/AUTHENTICATION.md` 참조.

| 메서드 | 경로 | 권한 | 설명 |
|---|---|---|---|
| GET | `/api/v1/version` | (토큰만) | 서버 버전 — 확장이 자기 버전과 비교해 업데이트 안내. 응답 `{"version": "..."}` |
