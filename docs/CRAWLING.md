...
- 페이지 간 최소 간격(`--delay`, 기본 5초)을 두어 대상 서버에 부담을 주지
  ...
  재시도할 수 있다.
- 옵션 기본값(최대 페이지 수·깊이·간격)은 대시보드 **시스템** 화면의 "사이트
  아카이브 기본값"에서, 허용 상한(최대값)과 재시도 대기 시간은 "사이트 아카이브
  최대값"에서 바꿀 수 있다. 기본값은 상한 이내로 자동 조정되고, 새 아카이빙
  화면 입력칸도 이 상한을 넘지 못한다(서버에서도 재검증). 재시도 대기는 쉼표로
  구분한 초 목록(예: `300, 900`)이고 대기 횟수 + 1 이 페이지당 최대 시도
  횟수다 — 진행 중인 크롤에도 즉시 적용된다.
