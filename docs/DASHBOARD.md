...
- **사이트 상세** (site) — `/sites/{id}`. 사이트에 속한 페이지 목록(스냅샷
  ...
  있던 페이지 행 동작이 여기로 왔다, 100개 단위 페이징 `?page=`)과 크롤
  ...
  자연히 사라진다. 페이지 행이 생기기 전에 실패한 신규 URL 은 소속
  사이트를 알 수 없어 포함하지 않는다 (크롤 실패는 크롤 진행 화면 참조). 사이트 삭제(`POST /sites/{id}/delete`,
...
- **시스템** (system) — 백업/복원·내보내기/가져오기·저장공간 최적화
  (`POST /system/compact`, `wccg compact` 와 동일 — 압축 변환 + 자원 참조
  백필 + 고아 공유 자원 정리. 대상이 없으면 버튼 비활성화)·가입 설정(회원
  가입 허용 여부 + 가입 초기 권한
