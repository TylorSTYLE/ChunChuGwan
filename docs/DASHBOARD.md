...
- **사이트 상세** (site) — `/sites/{id}`. 사이트에 속한 페이지 목록(스냅샷
  ...
  없으면 섹션을 숨기고, 50개를 넘으면 전체 문서 화면 링크로 안내한다.
...
- **사용자** (users) — `/system/users`, 헤더 "관리자" 드롭다운의 항목, 관리자
  전용 사용자 관리. 권한 조정(권한없음 가입자
...
- **사람 확인 필요** (needs_human) — `/archive/needs-human`, 관리자 전용.
  `WCCG_LIVE_CHALLENGE=on` 일 때만 — 자동(스텔스 엔진)으로 통과하지 못한
  인터랙티브 챌린지가 있으면 사람이 직접 풀어 통과시킨다. 발견성을 위해 네
  군데서 안내한다(모두 관리자+기능 켜짐일 때만): (1) **목록 화면의 상태 배지**
  — 진행 중 작업이 사람 대기로 전환되면 `/archives`·사이트 상세의 "아카이빙 중"
  배지가 라이브 화면으로 가는 "사람 확인 대기"(amber 알림) 링크로 바뀐다.
  서버 렌더라 JS·브라우저 설정과 무관하게 보이고, 폴링은 needs_human 변화도
  감지해 새로고침한다. (2) 헤더에 "사람 확인" 메뉴가 **상시** 표시되고(대기 건이
  있으면 "(n)" 배지·알림 색), (3) 어느 화면에 있든 대기 작업이 생기면 상단에
  전역 배너가 떠 바로 처리 화면으로 보낸다(`/archive/active` 폴링이 관리자+
  기능 켜짐일 때 needs_human 을 함께 내려보냄 — JS 의존이라 보조 수단), (4) 대기
  목록 + "처리" 버튼으로 라이브 화면 진입. 사람 처리 창(`WCCG_LIVE_CHALLENGE_TIMEOUT_SECONDS`, 기본
  5분)을 놓쳐 실패한 작업은 아카이빙 로그(`/logs`)에서 재시도하면 라이브
  세션이 다시 열린다(자동 백오프 재시도도 동일). 기능은 켰는데 캡처 엔진이
  스텔스(patchright/headful)가 아니면 라이브가 비활성이며, worker 가 그 이유를
  시스템 로그(`/system/logs`)에 한 번 경고로 남긴다.
- **라이브 챌린지 처리** (live) — `/archive/jobs/{id}/live`, 관리자 전용.
  ...
  다른 관리자는 보기 전용. "화면 갱신"·"취소"(`POST /live/cancel`) 제공.
  통과하면(`/live/state` 폴링) 자동으로 캡처가 이어지고 화면이 닫힌다.
