...
- **사람 확인 필요** (needs_human) — `/archive/needs-human`, 관리자 전용.
  ...
  인터랙티브 챌린지가 있으면 사람이 직접 풀어 통과시킨다. 발견성을 위해 세
  군데서 안내한다: (1) 헤더에 "사람 확인" 메뉴가 **상시** 표시되고(대기 건이
  있으면 "(n)" 배지·알림 색), (2) 어느 화면에 있든 대기 작업이 생기면 상단에
  전역 배너가 떠 바로 처리 화면으로 보낸다(`/archive/active` 폴링이 관리자+
  기능 켜짐일 때 needs_human 을 함께 내려보냄), (3) 대기 목록 + "처리" 버튼으로
  라이브 화면 진입. 사람 처리 창(`WCCG_LIVE_CHALLENGE_TIMEOUT_SECONDS`, 기본
