...
- **현황** (dashboard) — 첫 화면 `/`(= `/dashboard`). 본문은 사이트·
  ...
  태그 뱃지가 붙는다.
- **목록** (index) — `/archives`, 헤더 "아카이브" 드롭다운의 "아카이브 사이트 목록". 사이트(서브도메인) 단위
...
- **사이트 로그인 자격증명** (site_credentials) — `/sites/{id}/credentials`,
  ...
  사용)은 이후 단계.
- **문서** (documents) — `/documents`, 헤더 "아카이브" 드롭다운의 "전체 문서(파일)". 아카이브된 페이지들이
...
- **사이트 아카이브 진행** (crawl) — `/crawls/{id}`. 상태 카드(실패 재시도
  ...
  여기로 온다.
- **스케줄** (schedules) — `/schedules`, 헤더 "아카이브" 드롭다운의 "스케줄". 페이지 스케줄과
...
- **시스템** (system) — `/system`, 헤더 "설정" 드롭다운의 "시스템 설정" 항목, 관리자 전용.
