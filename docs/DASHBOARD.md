...
- **목록** (index) — `/archives`, 헤더 메뉴 "목록". 사이트(서브도메인) 단위
  ...
  회차 수·마지막 활동·자동(스케줄 수)·진행 중 뱃지, 사이트 텍스트 필터.
...
- **사이트 상세** (site) — `/sites/{id}`. 사이트에 속한 페이지 목록(스냅샷
  수·마지막 활동·자동 주기·재아카이빙/삭제 버튼 — 목록 화면에 있던 페이지
  행 동작이 여기로 왔다)과 크롤 회차 목록(`/crawls/{id}` 링크, 상태·실패
  뱃지), 크롤 스케줄 요약. 사이트 삭제(`POST /sites/{id}/delete`,
