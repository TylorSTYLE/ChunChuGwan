...
화면 23개의 라우트·권한·세부 동작 레퍼런스. UI 스타일·i18n 규칙 등
...
- **타임라인** (timeline) — `/archive/sites/{id}/page/{pageId}`. 헤더에 소속 사이트 링크
  (`/archive/sites/{id}`)와 로컬 네트워크 태그 뱃지를 보여준다. 스냅샷 행마다
...
- **감사 로그** (audit) — `/log/audit` (`view_audit_logs` 권한, 기본 admin), 헤더
  ...
  종류·요청자·기간 필터 + 페이징. 아카이빙(새/재아카이빙·API), 아카이브 열람
...
- **시스템 설정** (system) — `/system/general`, 헤더 "설정" 드롭다운의 "시스템 설정" 항목, 관리자 전용.
  ...
  확장 자격증명·문서 아카이브·로컬 네트워크 태그)·사용자 설정(가입·이메일 본인 인증)·
...
- **사용자** (users) — `/system/users`, 헤더 "설정" 드롭다운의 "사용자 관리" 항목,
  ...
  화면엔 권한 열도 없다(권한 묶음 내용은 권한 그룹 화면에서 편집). 각 행에서
