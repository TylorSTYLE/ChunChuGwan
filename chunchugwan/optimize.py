...
def pending_counts() -> tuple[int, int]:
    """(압축 변환 대상 스냅샷 수, 참조 백필 대상 스냅샷 수).

    시스템 화면이 최적화 버튼의 노출/대상 표시를 판정하는 기준 — 둘 다 0 이면
    실행할 것이 없다 (이후의 고아는 삭제 시 GC 가 즉시 정리한다).
    """
    with db.connect() as conn:
        unindexed = db.count_unindexed_snapshots(conn)
    return resources.compactable_count(), unindexed
