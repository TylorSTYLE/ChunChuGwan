...
const LAST_TAB_KEY = "last_tab";      // popup 마지막 선택 탭 (연결 시 복원, 해제 시 제거)
const STATUS_BADGE_PREF_KEY = "status_badge_enabled"; // 아카이브 상태 배지 토글 (기본 on)

// 빠른 캡처 컨텍스트 메뉴 id (연결 시에만 노출 — setConnectedUI 가 생성/제거).
const MENU_ARCHIVE_PAGE = "ccg-archive-page";
const MENU_CAPTURE_PAGE = "ccg-capture-page";
const MENU_ARCHIVE_LINK = "ccg-archive-link";
...
// 런타임 401(토큰 만료·use_api_keys 회수)이면 자동 연결 해제한다. connect 의 토큰
// 검증 호출은 "방금 입력한 토큰 무효"라 자동 해제 대상이 아니므로 noAutoLogout 으로 부른다.
async function apiFetch(path, { method = "GET", body = null, noAutoLogout = false } = {}) {
  ...
  if (resp.status === 401 && !noAutoLogout) await handleAuthLost();
...
async function connect({ base_url, token }) {
  ...
  // 가벼운 조회로 토큰 검증 + 권한 추론. 검증 401 은 "방금 입력한 토큰 무효"라
  // 자동 연결 해제(handleAuthLost) 대상이 아니므로 noAutoLogout 으로 호출한다.
  const view = await apiFetch("/api/v1/pages?url=https://example.invalid/", { noAutoLogout: true });
  if (view.status === 401) {
    await disconnect();
  ...
  await setConnectedUI(true); // 연결됨 — 우클릭 메뉴·상태 배지 활성
  return { ok: true, prefix: tok.slice(0, 10), can_view: canView };
}

async function disconnect() {
  // 토큰·연결 정보·추적·마지막 탭을 비운다 (notify_enabled·status_badge 토글은 보존).
  await chrome.storage.local.remove([...STORE_KEYS, WATCH_KEY, TARGETS_KEY, LAST_TAB_KEY]);
  await chrome.alarms.clear(POLL_ALARM);
  await updateBadge([]);
  await setConnectedUI(false); // 해제됨 — 우클릭 메뉴·상태 배지 제거
  return { ok: true };
}
...
// ---- 연결 상태 UI 게이팅 + 자동 연결 해제 ----
//
// 빠른 캡처(단축키·우클릭)와 상태 배지는 개인 API Key 로 인증(연결)된 동안에만
// 의미가 있다. 연결되면 우클릭 메뉴를 만들고, 해제되면 메뉴·상태배지를 거둔다.
// 명령을 주고받다 런타임 401(토큰 만료·use_api_keys 회수)이 나면 자동으로 해제한다.

const MENU_DEFS = [
  { id: MENU_ARCHIVE_PAGE, contexts: ["page"], key: "ctx_archive_page" },
  { id: MENU_CAPTURE_PAGE, contexts: ["page"], key: "ctx_capture_page" },
  { id: MENU_ARCHIVE_LINK, contexts: ["link"], key: "ctx_archive_link" },
];

// 연결 상태를 UI 에 반영. connected 면 우클릭 메뉴 생성, 아니면 메뉴·상태배지 제거.
async function setConnectedUI(connected) {
  try {
    await chrome.contextMenus.removeAll();
    if (connected) {
      for (const m of MENU_DEFS) {
        chrome.contextMenus.create({ id: m.id, contexts: m.contexts, title: msg(m.key) });
      }
    }
  } catch (e) { /* contextMenus 미지원 — 무시 */ }
  if (!connected) await clearStatusBadgeAll();
}

// 런타임 401(토큰 만료·권한 회수) — 자동 연결 해제 + 팝업에 알림(열려 있으면).
async function handleAuthLost() {
  const { token } = await getConfig();
  if (!token) return; // 이미 해제됨 — 중복 방지
  await disconnect();
  chrome.runtime.sendMessage({ type: "auth_lost" }).catch(() => {});
}

// 모든 탭의 상태 배지(per-tab override)를 제거해 전역(추적) 배지로 폴백시킨다.
async function clearStatusBadgeAll() {
  try {
    const tabs = await chrome.tabs.query({});
    for (const t of tabs) {
      if (t.id != null) await chrome.action.setBadgeText({ tabId: t.id, text: null });
    }
  } catch (e) { /* 무시 */ }
}
...
// ---- 빠른 캡처 (단축키·우클릭 — 팝업 없이) ----
//
// 결과를 볼 팝업이 없으므로 notify 토글과 무관하게 즉시 알림으로 피드백한다
// (사용자가 방금 누른 직접 액션의 결과). 서버 위임 아카이브는 대상 host 권한이
// 필요 없고(서버가 캡처), 브라우저 캡처만 *://*/* 권한이 있어야 동작한다.

async function quickNotify(title, message, targetUrl) {
  const notifId = `wccg:quick:${Date.now()}`;
  if (targetUrl) await setTarget(notifId, targetUrl);
...
// 현재(또는 우클릭한 링크) URL 을 서버 위임 아카이브 — host 권한 불필요.
async function quickArchiveServer(url) {
  if (!isHttpUrl(url)) {
    await quickNotify(msg("notif_quick_failed"), msg("err_unsupported_page"));
    return;
  }
  const { token } = await getConfig();
  if (!token) { await quickNotify(msg("notif_quick_failed"), msg("msg_need_connect")); return; }
  const res = await apiFetch("/api/v1/archive", { method: "POST", body: { url } });
  if (res.ok && res.data) {
    await trackJob(res, url);
    const { base_url } = await getConfig();
    await quickNotify(
      msg("notif_quick_archived"), shortUrl(url),
      base_url ? `${base_url}/extension/go?url=${encodeURIComponent(url)}` : null,
    );
  } else if (res.status !== 401) { // 401 은 handleAuthLost 가 처리
    await quickNotify(msg("notif_quick_failed"), shortUrl(url));
  }
}

// 현재 페이지를 브라우저에서 직접 캡처(ingest). *://*/* 권한이 이미 있어야 한다
// (비팝업 컨텍스트에선 권한 요청 불가) — 없으면 팝업을 열어 안내한다.
async function quickCaptureBrowser(tabId, url) {
  ...
  if (!(await chrome.permissions.contains({ origins: ["*://*/*"] }))) {
    await quickNotify(msg("notif_quick_failed"), msg("notif_need_permission"));
    if (typeof chrome.action.openPopup === "function") {
      try { await chrome.action.openPopup(); } catch (e) { /* 127 미만·실패 — 무시 */ }
    }
    return;
  }
  const res = await captureAndIngest({ url, tabId });
  if (res && res.ok && res.data) {
    await quickNotify(
      msg("notif_capture_done"),
      msg(OUTCOME_KEY[res.data.status] || "notif_outcome_done") + " — " + shortUrl(url),
    );
  } else if (res && res.needs_network_tag) {
    await quickNotify(msg("notif_quick_failed"), msg("notif_need_tag"));
...
async function fetchResources(urls, onProgress) {
  const map = {};
  let total = 0;
...
async function captureFullPage(tabId, onProgress) {
  ...
  try {
    ...
    // 재측정(지연 로딩·고정요소 변경 반영) 후라야 정확한 구간 수 — 그제야 total 보고.
    const segTotal = Math.max(1, Math.ceil(fullH / viewH));
    if (onProgress) onProgress(0, segTotal);
    ...
    for (let i = 0; i < MAX_CAPTURE_SEGMENTS; i++) {
      if (onProgress) onProgress(Math.min(i + 1, segTotal), segTotal);
...
async function uploadIngest(form) {
  ...
  if (resp.status === 401) await handleAuthLost();
  return { ok: resp.ok, status: resp.status, data };
}

// 브라우저 캡처 진행 단계를 팝업에 푸시한다 (열려 있을 때만 — 수신자 없으면 무시).
function pushProgress(phase, done, total) {
  chrome.runtime.sendMessage({ type: "capture_progress", phase, done, total }).catch(() => {});
}
...
async function captureAndIngest(payload) {
  ...
  const { map, incomplete: resIncomplete } = await fetchResources(
    grab.resources || [], (d, t) => pushProgress("resources", d, t),
  );
  ...
  const shotB64 = await captureFullPage(tabId, (d, t) => pushProgress("screenshot", d, t));
  pushProgress("documents");
...
async function checkVersion() {
  ...
  // 서버가 아는 확장 버전(extension_version)과 비교 — 서버 앱 버전과 독립.
  // 구버전 서버 호환: extension_version 이 없으면 비교 생략(오탐 방지).
  const latest = res && res.ok && res.data && res.data.extension_version;
...
const HANDLERS = {
  ...
  // 아카이브 상태 배지 토글 (기본 on). 끄면 모든 탭 배지 정리, 켜면 활성 탭 갱신.
  getStatusBadgePref: async () => ({ on: await statusBadgeEnabled() }),
  setStatusBadgePref: async (m) => {
    await setStatusBadgePref(!!(m.payload && m.payload.on));
    return { ok: true };
  },
...
// ---- 아카이브 상태 배지 (현재 페이지가 아카이브됐는지 툴바 아이콘에 표시) ----
//
// /api/v1/pages?url= 로 여부만 조회한다 ("변경됨"은 라이브 해시를 서버 정규화와 동일
// 계산할 수 없어 제외). per-tab 배지(✓)는 전역(추적 개수) 배지를 덮으므로(MV3),
// 추적이 없을 때만 그린다. 매 탭 전환 조회 비용은 단기 캐시 + 디바운스로 줄인다.

const STATUS_BADGE_COLOR = "#3a6ea5";  // 상태 ✓ 색 (추적 개수 배지 #1d6e56 와 구분)
const STATUS_CACHE_TTL_MS = 60000;     // URL 별 아카이브 여부 캐시 수명
const STATUS_DEBOUNCE_MS = 400;        // 탭 전환·SPA URL 변경 폭주 흡수
const statusCache = new Map();         // url → { archived, ts }
const statusTimers = new Map();        // tabId → setTimeout id

async function statusBadgeEnabled() {
  const c = await chrome.storage.local.get(STATUS_BADGE_PREF_KEY);
  return c[STATUS_BADGE_PREF_KEY] !== false; // 미설정 기본 on
}

async function setStatusBadgePref(on) {
  await chrome.storage.local.set({ [STATUS_BADGE_PREF_KEY]: on });
  if (on) await refreshActiveTabBadge();
  else await clearStatusBadgeAll();
}

// 탭 전환·로드마다 호출 — 디바운스해 조회 폭주를 막는다.
function scheduleStatusBadge(tabId) {
  if (tabId == null) return;
  const prev = statusTimers.get(tabId);
  if (prev) clearTimeout(prev);
  statusTimers.set(tabId, setTimeout(() => {
    statusTimers.delete(tabId);
    refreshStatusBadgeFor(tabId);
  }, STATUS_DEBOUNCE_MS));
}

async function refreshStatusBadgeFor(tabId) {
  let tab;
  try { tab = await chrome.tabs.get(tabId); } catch (e) { return; } // 닫힌 탭
  await refreshStatusBadge(tabId, tab && tab.url);
}

// 현재 활성 탭의 상태 배지 갱신 (연결 직후·추적 종료 후 복원용).
async function refreshActiveTabBadge() {
  try {
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    if (tab && tab.id != null) await refreshStatusBadge(tab.id, tab.url);
  } catch (e) { /* 무시 */ }
}

// url 의 아카이브 여부를 per-tab 배지로. 토글 off·미연결·비-http·추적중이면 그리지 않는다.
async function refreshStatusBadge(tabId, url) {
  if (tabId == null) return;
  if (!(await statusBadgeEnabled())) return;
  if (!isHttpUrl(url)) { await setTabBadge(tabId, ""); return; }
  const { token } = await getConfig();
  if (!token) { await setTabBadge(tabId, ""); return; }           // 미연결
  // 추적(작업 진행) 중이면 전역 개수 배지를 우선 — per-tab override 를 만들지 않는다.
  if ((await getWatch()).length > 0) { await setTabBadge(tabId, null); return; }
  const archived = await isArchived(url);
  if (archived == null) { await setTabBadge(tabId, ""); return; } // 권한 없음·오류 — 비움
  await setTabBadge(tabId, archived ? "✓" : "");
  try {
    await chrome.action.setTitle({
      tabId, title: msg(archived ? "badge_tooltip_archived" : "badge_tooltip_not"),
    });
  } catch (e) { /* 무시 */ }
}
...
async function setTabBadge(tabId, text) {
  try {
    if (text) await chrome.action.setBadgeBackgroundColor({ tabId, color: STATUS_BADGE_COLOR });
    await chrome.action.setBadgeText({ tabId, text }); // text:null 이면 per-tab override 제거
  } catch (e) { /* 무시 */ }
}

// url 이 아카이브됐는지 (단기 캐시 + /api/v1/pages?url=). null = 알 수 없음(미연결·권한·오류).
async function isArchived(url) {
  const hit = statusCache.get(url);
  if (hit && Date.now() - hit.ts < STATUS_CACHE_TTL_MS) return hit.archived;
  const res = await apiFetch("/api/v1/pages?url=" + encodeURIComponent(url));
  if (!res.ok || !res.data) return null;
  const archived = Array.isArray(res.data.pages) && res.data.pages.length > 0;
  statusCache.set(url, { archived, ts: Date.now() });
  return archived;
}
...
async function pollStatus() {
  ...
  if (remaining.length === 0) {
    await chrome.alarms.clear(POLL_ALARM);
    await refreshActiveTabBadge(); // 추적 종료 — 활성 탭 상태 배지 복원
  }
}

// SW 재기동·설치 대비 — 추적 중이면 알람·배지 복원 + 연결 상태에 따라 메뉴/게이팅 복원.
async function rearm() {
  const watch = await getWatch();
  await updateBadge(watch);
  if (watch.length) await ensureAlarm();
  const { token } = await getConfig();
  await setConnectedUI(!!token); // 컨텍스트 메뉴는 영속이 아니라 매 기동 재생성
  if (token) await refreshActiveTabBadge(); // 상태 배지 복원
}
...
// 빠른 캡처 — 단축키. 연결 안 됐으면 quick* 헬퍼가 "연결 먼저" 알림으로 가드한다.
chrome.commands.onCommand.addListener(async (command) => {
  const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
  if (!tab || tab.id == null) return;
  if (command === "archive-page-server") await quickArchiveServer(tab.url);
  else if (command === "capture-page-browser") await quickCaptureBrowser(tab.id, tab.url);
});

// 빠른 캡처 — 우클릭 메뉴 (연결 시에만 setConnectedUI 가 메뉴를 만든다).
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  if (info.menuItemId === MENU_ARCHIVE_LINK) await quickArchiveServer(info.linkUrl);
  else if (info.menuItemId === MENU_ARCHIVE_PAGE) await quickArchiveServer(tab && tab.url);
  else if (info.menuItemId === MENU_CAPTURE_PAGE) {
    await quickCaptureBrowser(tab && tab.id, tab && tab.url);
  }
});

// 활성 탭이 바뀌거나 로드 완료되면 상태 배지를 갱신한다 (기능 2).
chrome.tabs.onActivated.addListener(({ tabId }) => scheduleStatusBadge(tabId));
chrome.tabs.onUpdated.addListener((tabId, info) => {
  if (info.status === "complete" || info.url) scheduleStatusBadge(tabId);
});
