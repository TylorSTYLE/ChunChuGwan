...
막지 않는다 (호출부가 None 처리). 결과는 (host, port) 단위 TTL 캐시 —
크롤이 같은 호스트의 페이지마다 핸드셰이크를 반복하지 않는다.
"""
...
# (host, port) → (만료 monotonic, 파싱 결과 또는 None). 크롤이 같은 호스트를
# 페이지마다 핸드셰이크하지 않게 한다 — 실패(None)도 캐시해 응답 없는
# 호스트의 connect 타임아웃이 페이지마다 반복되는 것을 막는다.
...
def fetch_certificate_info(url: str) -> dict | None:
    ...
    결과는 (host, port) 단위로 TTL 캐시된다 (CERT_CACHE_TTL_SECONDS) —
    캐시 적중이어도 호출부의 last_seen 갱신은 그대로 일어난다.
    """
    parts = urlsplit(url)
    ...
    try:
        ...
        info = None
    with _cache_lock:
        _cache[(host, port)] = (now + config.CERT_CACHE_TTL_SECONDS, info)
